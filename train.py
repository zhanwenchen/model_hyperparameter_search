...
def train(identifier):
    """Train all models that match """
    models = glob.glob(os.path.join(trained_models_folder, str(identifier) + '_created'))
    for model_folder in models:
        model_params_path = os.path.join(model_folder, 'model_params.txt')
        print('train.py: training model', os.path.basename(model_folder),
              'with hyperparams')

        # load and print model and training hyperparameters.
        ...
        if using_cuda == True:
            ...
            print('train.py: Not using CUDA')

        # Load primary training data
        num_samples = 10 ** 5
        dat_train = ApertureDataset(model_params['data_train'],
                                    num_samples, model_params['k'],
                                    model_params['data_is_target'])
